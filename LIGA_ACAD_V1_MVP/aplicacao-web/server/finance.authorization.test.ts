...
import { canDeactivateProperty, ensurePropertyOwnership } from "./routers/finance";
...
describe("finance authorization", () => {
  ...
  it("libera a remoção apenas para gestor, administrador de perfil ou administrador técnico", () => {
    expect(canDeactivateProperty("gestor", "user")).toBe(true);
    expect(canDeactivateProperty("administrador", "user")).toBe(true);
    expect(canDeactivateProperty("produtor", "user")).toBe(false);
    expect(canDeactivateProperty("consultor", "user")).toBe(false);
    expect(canDeactivateProperty(null, "admin")).toBe(true);
  });
});
